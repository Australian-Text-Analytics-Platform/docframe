...
def test_auto_document_detection():
    ...
    data4 = {"id": [1, 2, 3], "score": [0.1, 0.2, 0.3]}
    ...
    for col, avg_len in sorted(
        lengths_analysis.items(), key=lambda x: x[1], reverse=True
    ):
    ...
    assert df1.active_document_name == "content", (
        "Should auto-detect 'content' as document column"
    )
    assert guessed == "content", "guess_document_column should return 'content'"
    assert df3.active_document_name == "text", (
        "Should auto-detect 'text' as document column"
    )
