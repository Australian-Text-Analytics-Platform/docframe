...
import docframe as docf
from docframe.core.text_utils import quotation_elements
...
class TestComputeTokenFrequencies:
    ...
    def test_basic_functionality(self):
        df1 = docf.DocDataFrame({"text": ["hello world", "hello there"]})
        df2 = docf.DocDataFrame({"text": ["world peace", "hello world"]})
        frames = {"frame1": df1, "frame2": df2}
        freqs, stats = docf.compute_token_frequencies(frames)
    ...
    def test_stop_words_filtering(self):
        df1 = docf.DocDataFrame({"text": ["the hello world", "the hello there"]})
        df2 = docf.DocDataFrame({"text": ["world peace the", "the hello world"]})
        frames = {"frame1": df1, "frame2": df2}
        freqs, _ = docf.compute_token_frequencies(frames, stop_words=["the"])
    ...
    def test_multiple_stop_words(self):
        df1 = docf.DocDataFrame({"text": ["the quick brown fox", "a lazy dog"]})
        freqs, _ = docf.compute_token_frequencies(
    ...
    def test_mixed_frame_types(self):
        eager = docf.DocDataFrame({"text": ["hello world", "hello there"]})
        lazy = docf.DocDataFrame({
            "text": ["world peace", "hello world"]
        }).to_doclazyframe()
        freqs, _ = docf.compute_token_frequencies({"eager": eager, "lazy": lazy})
        assert set(freqs["eager"].keys()) == set(freqs["lazy"].keys())

    def test_single_frame(self):
        df = docf.DocDataFrame({"text": ["hello world", "hello there"]})
        freqs, stats = docf.compute_token_frequencies({"single": df})
    ...
    def test_empty_frames_dict(self):
        with pytest.raises(ValueError):
            docf.compute_token_frequencies({})

    def test_invalid_frame_type(self):
        with pytest.raises(TypeError):
            docf.compute_token_frequencies({"bad": pl.DataFrame({"text": ["x"]})})

    def test_empty_documents(self):
        df1 = docf.DocDataFrame({"text": ["", ""]})
        df2 = docf.DocDataFrame({"text": ["hello world", ""]})
        freqs, _ = docf.compute_token_frequencies({"empty": df1, "mixed": df2})
        assert all(v == 0 for v in freqs["empty"].values())
        assert freqs["mixed"]["hello"] == 1

    def test_complex_text_content(self):
        df1 = docf.DocDataFrame({"text": ["Hello, world!", "It's a beautiful day."]})
        df2 = docf.DocDataFrame({
            "text": ["World peace is possible.", "Hello everyone!"]
        })
        freqs, _ = docf.compute_token_frequencies({"complex1": df1, "complex2": df2})
    ...
    def test_very_long_token_lists(self):
        ...
        freqs, _ = docf.compute_token_frequencies({
            "long1": docf.DocDataFrame({"text": [long_text1]}),
            "long2": docf.DocDataFrame({"text": [long_text2]}),
        })
    ...
    def test_auto_detected_document_column(self):
        ...
        freqs, _ = docf.compute_token_frequencies({"auto": df})
        assert (
    ...
    def test_case_insensitive_stop_words(self):
        df = docf.DocDataFrame({"text": ["The Hello WORLD", "the world peace"]})
        freqs, _ = docf.compute_token_frequencies({"case": df}, stop_words=["the"])
        assert (
...
class TestQuotationElements:
    """Tests for heuristic quotation extraction"""

    def test_basic_quote_and_verb_before(self):
        text = 'John Smith said, "Hello world." It was a sunny day.'
